...
export class SimulatorRendererContainer implements BuiltinSimulatorRenderer {
  ...
  run() {
    ...
    let renderElement = <SimulatorRendererView rendererContainer={this} />;

    const { ConfigProvider } = (window as any)['antd'];
    if (ConfigProvider) {
      renderElement = (
        <ConfigProvider prefixCls="catl" {...(window as any)['__dicc_themes']}>
          <SimulatorRendererView rendererContainer={this} />
        </ConfigProvider>
      );
    }

    reactRender(renderElement, container);
